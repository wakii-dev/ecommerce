...
1. **7 OpenAPI 3.1 specs** trong `contracts/openapi/`: `identity.yaml`, `catalog.yaml`, `cart.yaml`, `ordering.yaml`, `payment.yaml`, `inventory.yaml`, `notification.yaml`. Pins BẮT BUỘC (spec §6.1 — checklist review từng item):
   ...
   - `ordering.yaml` bổ sung: `GET /api/ordering/coupons/public` (danh sách coupon active cho coupon center — code gợi ý, type, giá trị tối thiểu, window; KHÔNG lộ usage nội bộ); **hóa đơn (D18): `GET /api/ordering/me/orders/{id}/invoice` + `GET /api/ordering/admin/orders/{id}/invoice` → `application/pdf` (InvoiceProvider SPI, số HĐ tuần tự)**.
