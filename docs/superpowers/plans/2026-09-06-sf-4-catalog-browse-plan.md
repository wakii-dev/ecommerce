...
- [ ] Verify: IT create product qua service với comparePrice (→ Task 8b) + flash + variants có priceDelta → ProductCard trả đủ discountPercent + flashSaleEndsAt; variant priceDelta đúng hiệu *(verify bằng ProductApiTest seed qua repository — cùng surface đọc, admin write chưa tồn tại ở Task 3/8)*
