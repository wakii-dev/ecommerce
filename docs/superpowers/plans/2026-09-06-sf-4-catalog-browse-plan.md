...
- [ ] **Controllers map FULL prefix `@RequestMapping("/api/catalog")`** (Conventions #11); gateway block `catalog` un-comment KHÔNG StripPrefix + comment line giải thích (Conventions #11) — verify `curl :8080/api/catalog/products` qua gateway khớp `curl :8082/api/catalog/products`
