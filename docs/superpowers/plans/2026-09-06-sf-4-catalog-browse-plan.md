...
- [ ] catalog-service IT bổ sung (nếu thiếu): admin guard — call `/api/catalog/admin/products` không token → 401; token customer role → 403; publish flow end-to-end: admin create → outbox row → (đã ở Task 5 IT); category có product/conchildren → DELETE 409; reviews/wishlist paths → default 404 (JSON Boot, không RFC 7807 — ghi nhận, không test shape); **PgFts-only IT dùng context riêng `elasticsearch.uri=` rỗng + tắt ES container properties (2 context config trong suite — thiết kế sẵn, không để ES container auto-start chung)**
