...
- Wave 2 (registry + preset + session) — thứ tự T2→T3→T10:
  - [ ] T2 `chrome-vite-preset-shared-singletons-1-line` — SHARED_SINGLETONS += chrome (ĐÚNG 1 dòng). Verify (P2 critic): grep anchor 1 dòng trong `packages/config/vite-preset.mjs` + `turbo run build` (mọi MFE build consume preset).
...
- Wave 3 (header/footer/theme) — **thứ tự trong wave BẮT BUỘC T4→T5→T7 (P2 critic — wave = review group, KHÔNG license chạy song song)**:
  - [ ] T4 `shell-header-adapter-final-state-sf1` — SiteHeader (**'use client' — P0 critic**; merge order props TRƯỚC registry SAU; aria `chrome.header.aria`='Trang chủ') + Header.tsx adapter + chrome.css (port header values tokens-only + KHÔNG `url()` — P1 critic + `.chrome-container`) + main.tsx import styles. **Phân định T4/T5 (P1 critic): T4 = component + adapter wiring; T5 sở hữu SSR contract (props path + test + evidence ACCEPTANCE 3).** Verify: unit render + build xanh.
  - [ ] T5 `site-header-ssr-able-slots-props-render-to-static-markup` — SSR contract: props path render server được + renderToStaticMarkup test (links trong HTML, sạch server) — evidence ACCEPTANCE 3.
  - [ ] T7 `theme-toggle-canonical-key-ecommerce-theme-boot-script` — chrome ThemeToggle + THEME_STORAGE_KEY/THEME_BOOT_SCRIPT/resolveTheme/storedThemeValue + main.tsx đổi nguồn + **xóa `apps/shell/src/ThemeToggle.tsx` (F2 — P0 critic: chuyển từ T4 sang ĐÂY, cùng commit với rewire main.tsx kẻo red build giữa wave)** + unit bảng case. Verify: build xanh + unit.
