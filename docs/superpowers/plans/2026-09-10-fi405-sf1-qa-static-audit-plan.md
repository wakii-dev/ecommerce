...
- [ ] Extract `${VAR:default}` từ `backend/services/*/src/main/resources/application*.yml` + `backend/gateway/src/main/resources/application*.yml` + MỌI `routes/*.yml` (A4) + `@Value("${VAR:default}")` annotations (regex, skip comment dòng) → model `{service, var, default, sourceFile:line}`
