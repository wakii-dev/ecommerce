...
| D18 | **Hóa đơn điện tử + email cảm ơn**: tự xuất hóa đơn PDF theo layout HĐ VN (mẫu số/Ký hiệu/số HĐ tuần tự theo (mẫu, ký hiệu, năm), người bán/mua, bảng hàng, **VAT breakdown** — giá đã bao gồm VAT, dòng tách `total × rate/(100+rate)`, rate từ env `INVOICE_VAT_RATE=10`, ghi "Bản demo" thay chữ ký số) qua **`InvoiceProvider` SPI** (default PDFBox self-issued; adapter VNPT/MISA sau khi có tài khoản NCC — extensibility). Download: my-orders (customer) + admin order detail. **Email cảm ơn** khi `order.confirmed`: lời cảm ơn + tóm tắt đơn + **đính kèm PDF hóa đơn** + link my-orders (notification gọi REST nội bộ ordering lấy PDF bytes) | **USER** + AGENT chốt SPI + VAT-inclusive breakdown |
...
| SF-9 | ordering saga + coupons + invoice | 3 | SF-5 | ordering-service + saga orchestrator (reserve sync all-or-nothing, compensation edges + late-payment refund — §3.3, fail-injection tests) + outbox wiring, coupons validate/usage-reserve, order state machine §3.6 + my-orders APIs **+ my-orders UI trong mfe-account (`pages/orders/*` — file-slice) + nút tải hóa đơn**, **hóa đơn PDF: `InvoiceProvider` SPI (PDFBox layout HĐ VN, VAT breakdown, số HĐ tuần tự) + APIs customer/admin (D18)** (~14) |
