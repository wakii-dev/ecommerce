...
**Storefront:** home (hero + flash deal + featured), PLP (grid, filter category/price/rating, sort, pagination), search (ES + fallback PG FTS + suggest), PDP (gallery, giá VND, tồn kho, variants đơn giản size/color, add-to-cart, reviews section + viết review, wishlist heart), cart (guest + login merge), checkout (địa chỉ, vận chuyển flat-fee demo, coupon, review đơn, Stripe test payment), xác nhận đơn + **email cảm ơn kèm hóa đơn PDF (D18)**, my orders (lịch sử + chi tiết + trạng thái + **tải hóa đơn PDF**), my wishlist, my reviews, coupon center.
...
| SF-12 | affiliate | 4 | SF-3, SF-6, SF-9 | `affiliate-service` (D20): registry + ref code + track clicks (storefront capture `?ref=` → cookie 30 ngày — edit nhỏ storefront-web), checkout truyền affiliate_code (edit nhỏ mfe-checkout), order.confirmed → ledger hoa hồng (rate per-affiliate), account dashboard affiliate (`pages/affiliate/*` slice), admin duyệt + rate + stats (live), compose/Makefile/gateway route + db_affiliate init (~11) |
...
**Parallelism:** T2 chạy 3 SF song song (SF-3/4/5), T3 chạy 4 SF song song (SF-6/7/8/9 — file sets rời nhau theo shared-file ownership phía trên), T4 chạy 2 SF song song (SF-11/12), T5 convergence — nhờ contracts freeze + append-only rules.
