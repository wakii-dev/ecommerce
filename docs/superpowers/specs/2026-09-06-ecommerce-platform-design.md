...
| D18 | **Hóa đơn điện tử + email cảm ơn**: tự xuất hóa đơn PDF theo layout HĐ VN (mẫu số/Ký hiệu/số HĐ tuần tự theo (mẫu, ký hiệu, năm), người bán/mua, bảng hàng, **VAT breakdown** — giá đã bao gồm VAT, dòng tách `total × rate/(100+rate)`, rate từ env `INVOICE_VAT_RATE=10`, ghi "Bản demo" thay chữ ký số). **PDF render = Python microservice riêng (USER): `invoice-service` (FastAPI + ReportLab, :8090, stateless — nhận full payload → trả PDF bytes; internal-only, không qua gateway)**. Số HĐ + dữ liệu hóa đơn = business truth thuộc ordering (Java) — `InvoiceProvider` SPI với default `HttpInvoiceProvider` gọi Python service; lỗi service → 503 rõ ràng (degraded như Stripe). Download: my-orders (customer) + admin order detail. **Email cảm ơn** khi `order.confirmed`: lời cảm ơn + tóm tắt đơn + **đính kèm PDF hóa đơn** + link my-orders (notification gọi REST nội bộ ordering lấy PDF bytes) | **USER** (Python service) + AGENT chốt SPI + stateless split + VAT-inclusive breakdown |
| D19 | **Open API module cho đối tác** (`partner-api`, Java, :8091, db_partner): namespace public `/open-api/v1/**` — auth **`X-API-Key`** (hash trong bảng partners, scopes + rate-limit per key), endpoints: catalog read (products/categories/search), `POST /orders` (đơn partner), `GET /orders/{id}`; **webhook**: partner đăng ký URL + secret — consume `order.*` → HMAC-SHA256 sign POST đến partner URL (retry + DLQ); **docs portal** springdoc tại `/open-api/v1/docs`. Contract `partner-api.yaml` freeze SF-2 | **USER** + AGENT chốt scope MVP |
...
├── services/
│   ├── invoice-service/                # 🐍 Python 3.12 + FastAPI + ReportLab (D18): stateless PDF renderer
│   │                                   #   POST /generate → PDF bytes; internal-only (:8090, không qua gateway)
│   ├── partner-api/                    # Open API module cho đối tác (D19): /open-api/v1/**, API key auth,
│   │                                   #   rate-limit, webhooks HMAC, docs portal (:8091, db_partner)
│   └── affiliate-service/              # Affiliate module (D20): ref code, attribution, ledger hoa hồng
│                                       #   (:8092, db_affiliate)
...
| invoice 🐍 Python | stateless PDF renderer (layout HĐ VN) — KHÔNG giữ business data | — | — | — (REST nội bộ từ ordering) |
| partner-api | Open API đối tác: partners/api keys/scopes, rate-limit, webhook HMAC delivery | db_partner | — (REST webhooks outbound) | order.* (webhook delivery) |
| affiliate | affiliate registry, ref tracking (clicks), ledger hoa hồng, payout ghi sổ | db_affiliate | — | order.confirmed (tạo ledger entry) |
...
**Storefront:** home (hero + flash deal + featured), PLP (grid, filter category/price/rating, sort, pagination), search (ES + fallback PG FTS + suggest), PDP (gallery, giá VND, tồn kho, variants đơn giản size/color, add-to-cart, reviews section + viết review, wishlist heart, **related products + recently viewed — D21**), cart (guest + login merge, **email bỏ quên giỏ — D21**), checkout (địa chỉ, vận chuyển flat-fee demo, coupon, **Stripe test HOẶC COD (D21)**, review đơn), xác nhận đơn + **email cảm ơn kèm hóa đơn PDF (D18)**, my orders (lịch sử + chi tiết + trạng thái + **tải hóa đơn PDF**), my wishlist, my reviews, coupon center, **quên mật khẩu (D21)**, **newsletter (D21)**, **GA4 (D21)**.

**Admin (mfe-admin, RBAC):** dashboard (KPI: doanh thu/đơn/top sản phẩm/low-stock + biểu đồ), products CRUD (table + form + ảnh **upload MinIO thật (D21)**, publish/draft, SEO fields), categories CRUD, coupons CRUD (%, fixed, window, usage limit), reviews moderation (queue duyệt/ẩn), orders (list, filter, detail, chuyển trạng thái theo §3.6: ship/deliver/cancel — KHÔNG có nút confirm, **tải hóa đơn**), users (list + role view), **audit log viewer (D21)**, **export CSV đơn/sản phẩm (D21)**, affiliates quản lý (D20). Dashboard data: ordering.yaml (revenue-by-day, orders-summary, top-products) + inventory.yaml (low-stock) — endpoint read-only, freeze tại SF-2, không BFF riêng.
...
12. **Hóa đơn + email cảm ơn (D18)**: đơn CONFIRMED → my-orders có nút tải hóa đơn PDF (mở được, đủ trường hóa đơn VN, số HĐ tăng dần qua các đơn); admin tải được hóa đơn cùng đơn; Mailpit có email cảm ơn **kèm file đính kèm PDF** đúng đơn.
13. **Open API đối tác (D19)**: curl với `X-API-Key` hợp lệ → `GET /open-api/v1/products` trả catalog; key sai/hết hạn → 401; docs portal `/open-api/v1/docs` mở được; partner có webhook → đổi trạng thái đơn nhận được POST HMAC-signed (verify bằng secret).
14. **Affiliate (D20)**: click link `?ref=<code>` → cookie attribution; mua hàng qua link đó → đơn CONFIRMED → affiliate dashboard hiện conversion + hoa hồng đúng rate; admin duyệt affiliate + đổi rate thấy ở dashboard; link không hợp lệ → không tracking, không lỗi.
15. **Essentials (D21)**: forgot password → email reset → đổi được mật khẩu; checkout chọn COD → đơn CONFIRMED không qua Stripe, admin vẫn thấy; upload ảnh trong admin form → ảnh hiện trên sản phẩm; bỏ giỏ không thanh toán (user có email) → nhận email nhắc; PDP có "Sản phẩm tương tự" + "Đã xem gần đây"; admin xem audit log; export CSV đơn; newsletter subscribe nhận welcome email; GA4 nhận pageview.
16. **Batch B (D22)**: RMA — tạo yêu cầu đổi trả trên đơn DELIVERED → admin duyệt → refund Stripe → tiền về test wallet; GHN — nhập địa chỉ → phí ship tính theo GHN, đơn có mã vận đơn tracking; loyalty — mua hàng được điểm, checkout dùng điểm giảm tiền; login bằng Google/Facebook được; bật 2FA → đăng nhập cần mã TOTP; sản phẩm hết hàng → đăng ký nhận email khi có lại; PWA cài được lên desktop; dark mode chuyển được; chat widget hiện.
...
| SF-9 | ordering saga + coupons + invoice | 3 | SF-5 | ordering-service + saga orchestrator (reserve sync all-or-nothing, compensation edges + late-payment refund — §3.3, fail-injection tests) + outbox wiring, coupons validate/usage-reserve, order state machine §3.6 + my-orders APIs **+ my-orders UI trong mfe-account (`pages/orders/*` — file-slice) + nút tải hóa đơn**, **hóa đơn (D18): số HĐ + dữ liệu hóa đơn thuộc ordering (Java) qua `InvoiceProvider` SPI — default `HttpInvoiceProvider` gọi `invoice-service` 🐍 Python (FastAPI + ReportLab, stateless renderer, :8090 internal) + APIs customer/admin** (~15) |
| SF-10 | convergence + ship | 5 | SF-6, SF-7, SF-8, SF-9, SF-11, SF-12 | Checkout **live wiring** (mfe-checkout → ordering thật), **notification-service (email cảm ơn + đính kèm PDF hóa đơn — D18; Mailpit) + log-service (Mongo `event_log` fan-in — D14)**, golden-path Playwright E2E + admin CRUD spec + **review-flow E2E (§5.6)** + saga fail spec (§5.7) + asserts §5.3 (admin tạo product → thấy trên storefront) / §5.4 (mỗi service standalone) / §5.5 (gateway 403 admin) / **§5.13 (partner API)** / **§5.14 (affiliate)**, deterministic seed (coupon code cố định, product names cho search, Stripe test cards 4242-success + 4000...0002-declined), profile `full` compose + static MFE hosting qua gateway + **final mounts: đủ 4 remote apps + shell host, full route table, `make dev` full-stack**, demo README + script, ADR hoàn thiện, perf/security sanity (~14) |
| SF-11 | partner Open API | 4 | SF-4, SF-9 | `partner-api` service (D19): partners/api_keys/scopes registry + `X-API-Key` auth + rate-limit per key, `/open-api/v1/**` (catalog read, POST /orders, GET /orders/{id}), webhook HMAC delivery (consume order.*, retry), springdoc docs portal, compose/Makefile/gateway route + db_partner init (~10) |
| SF-13 | essentials & polish | 6 | SF-3, SF-4, SF-6, SF-7, SF-9, SF-10 | **Batch A — D21**: password reset (identity + email), **COD payment** (adapter no-op + saga skip intent + checkout option), upload ảnh MinIO (compose + catalog admin endpoint + admin form), abandoned cart email (cart scheduler + notification), audit log viewer admin (đọc Mongo event_log qua log-service), recently viewed + related products (ES more_like_this), GA4/GTM env-driven, admin export CSV, newsletter, E2E regression essentials (~13) |
| SF-14 | commerce extensions | 6 | SF-7, SF-9, SF-10 | **Batch B — D22**: RMA đổi trả (trạng thái riêng trong ordering + Stripe refund qua adapter + emails + admin duyệt), GHN shipping (methods/fee theo địa chỉ + tracking), loyalty điểm (earn 1% + burn checkout) trong affiliate-service, admin RMA/loyalty pages (~13) |
| SF-15 | engagement & platform | 6 | SF-3, SF-4, SF-5, SF-10 | **Batch B — D22**: social login Google/Facebook (identity OAuth) + 2FA TOTP, stock alert ("nhắn tôi khi có hàng" + email restock), PWA manifest + service worker, dark mode (ui-kit tokens), live chat embed (env config), IT tests (~10) |
| SF-12 | affiliate | 4 | SF-3, SF-6, SF-9 | `affiliate-service` (D20): registry + ref code + track clicks (storefront capture `?ref=` → cookie 30 ngày — edit nhỏ storefront-web), checkout truyền affiliate_code (edit nhỏ mfe-checkout), order.confirmed → ledger hoa hồng (rate per-affiliate), account dashboard affiliate (`pages/affiliate/*` slice), admin duyệt + rate + stats (live), compose/Makefile/gateway route + db_affiliate init (~10) |
...
5. **order.confirmed fat payload:** `{order_id, user_id, email, items[{product_id, variant_id, qty, price}], subtotal, discount, total, coupon_code, affiliate_code nullable}` — đủ cho mọi consumer (notification/cart/catalog/log/partner/affiliate) không cần call-back HTTP.
...
**Exit criteria Tier 0-1 (pin):** SF-1 gate = `docker compose up` infra healthy + template service boot `/actuator/health` xanh (Testcontainers) + turbo build xanh + gateway smoke route 200. SF-2 gate = 10 OpenAPI specs validate + codegen compile + packages/auth unit tests xanh + **federation harness xanh** + USER đã chọn design direction (riêng — KHÔNG chặn contract freeze).
