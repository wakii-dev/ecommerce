...
- **Endpoints (đúng contract):**
  ...
  - `POST /api/cart/items` `{productId, variantId?, qty, allowOos?}` + query hint `?slug=` → **auto-create giỏ khi guest mới + Set-Cookie `cart_token` trên CHÍNH path này** (spec extension có chủ đích — không có nó giỏ guest bị mồ côi vì FE chỉ gọi POST /items); enrich snapshot; catalog **404 → reject 404** (product/variant không tồn tại), **catalog chết (timeout/mạng) → chấp nhận item KHÔNG enrichment** (unitPrice 0, `unavailable: true` ở GET sau — không tự xóa, không fake giá); variant OOS & !allowOos → **409**; response Cart mới nhất.
  ...
  - `POST /api/cart/merge` (JWT bắt buộc) `{cartToken}` — **body required theo contract** (cookie chỉ là leniency server-side khi body rỗng, không thay thế) → gộp dedupe line identity (cộng qty), enrich lại, xóa giỏ guest + expire cookie; guest không tồn tại → 404.
...
- **Confirmation `/order/confirmation`:** đọc order CONFIRMED từ sessionStorage (`ecommerce.last_order` — stub không có server persist) → cảm ơn + order summary + trạng thái "Đang xử lý" · rơi vào mà không có order → empty state.
- **CartBadge:** mount → `GET /api/cart` (404 → 0) → badge `Σ qty`; lắng nghe `ecommerce:cart-changed` → re-fetch; click → navigate /cart. Ẩn khi count = 0.
...
- `PdpBuyBox.tsx` nhận thêm `slug` prop từ page (thread qua mới — hiện PdpBuyBox chỉ nhận `{product, locale}`) rồi truyền xuống AddToCart.
