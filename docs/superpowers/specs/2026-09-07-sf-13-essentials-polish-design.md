...
- `NewsletterController @RequestMapping("/newsletter")`: `POST ""` → `{email}` → insert mới → outbox `user.newsletter_subscribed` `{email, subscribedAt}` → 200 `{status:"subscribed"}`; dup → **KHÔNG row mới, KHÔNG event** → 200 `{status:"already"}`.
- **Admin list tách controller riêng** (spec-critic P1 — tránh path rò rỉ): `AdminNewsletterController @RequestMapping("/admin")` method `/newsletter` → URL `/api/identity/admin/newsletter` (ROLE_ADMIN) → page `{items:[{email, createdAt}], page, size, total}`. (Đặt GET trong `/newsletter` controller sẽ thành `/api/identity/newsletter/admin` — KHÔNG khớp gateway admin-prefix `/api/identity/admin/**` lẫn identity guard `/admin/**` → rơi anyRequest.authenticated → mọi customer đọc được list.)
...
- `spi/CodPaymentAdapter implements PaymentProviderAdapter`: `createIntent` → no-op trả `AdapterIntent("cod:<orderId>", null, "REQUIRES_CONFIRMATION")` (clientSecret null — contract "NULL khi COD"); `voidIntent/refund` → no-op success record; `verifyWebhook` → throw UnsupportedOperation. **KHÔNG đăng ký bean** (spec-critic P0: payment-service inject MỘT `PaymentProviderAdapter` duy nhất — stripe/unconfigured; thêm bean thứ 2 = NoUniqueBeanDefinitionException). Class thuần, test instantiate trực tiếp; COD không đi qua createIntent (saga bỏ bước) — capture qua `CodCaptureService`.
- Capture (additive runtime, ADR): `POST /payment/cod/captures` `{orderId, amountVnd, idempotencyKey}` → insert/update PaymentIntent (`providerIntentId=cod:<orderId>`, status SUCCEEDED, stripe_status "succeeded") idempotent theo Idempotency-Key → KHÔNG emit payment.succeeded (tránh late-refund path) → trả 201.
...
- `OrderStatus.ALLOWED`: PENDING → {PAID, **CONFIRMED**, CANCELLED, FAILED} (path mới duy nhất). **Trạng thái đơn COD kết thúc = DELIVERED — KHÔNG thêm transition →PAID; "PAID" chỉ là nghĩa vụ thanh toán qua capture + order.paid** (BẬT: RMA SF-14 khoá theo DELIVERED, không vỡ). `onInventoryCommitted` giữ guard PAID (COD confirm trước khi inventory.committed về — event đến lúc DELIVERED, guard no-op an toàn).
- `AdminOrderController.deliver` COD: **capture TRƯỚC transition** (spec-critic P1: capture sau transition fail → deliver retry bị guard SHIPPED 409 chặn, không redrive được). Thứ tự: capture REST (idempotency key `cod-capture:<orderId>`) OK → tx transition SHIPPED→DELIVERED + outbox `order.paid` `{orderId, paymentIntentId:"cod:<orderId>", paidAt}` → inventory commit lúc giao. Capture fail → 502, order còn SHIPPED → operator deliver lại (capture idempotent).
- `cancelByAdmin`/`refundSafely`: guard skip refund khi `paymentMethod=cod` (chưa thu tiền — không có gì hoàn; tránh refund(null intentId) → lỗi requeue).
- inventory KHÔNG đụng (không phải file SF-13): bind `order.paid` sẵn. Hạn chế đã biết (ADR): reservation TTL 30' release trong window CONFIRMED→DELIVERED → COD không giảm tồn kho ở quy mô demo — chấp nhận vì inventory ngoài scope SF-13, sửa sau nếu cần.

### 2.3 catalog-service (A3 upload + A6b related + A7b CSV) + ordering CSV
- Upload: dep `io.minio:minio:8.5.x`. `MinioConfig`: client endpoint `${catalog.minio.endpoint:http://localhost:9000}` (bean luôn tạo — lazy; bucket-initializer `@ConditionalOnProperty catalog.minio.enabled` — IT không thấy MinIO vẫn boot), bucket `products` (tự tạo nếu thiếu, public-read anonymous download). `POST /api/catalog/admin/uploads` (ADMIN, trong AdminProductController) — multipart field **`image`** (contract WINS — không phải `file`), validate ≤5MB + ext/content-type ∈ {jpg,jpeg,png,webp} → **object key `<uuid>.<ext>` trong bucket `products`** (spec-critic P1: prefix `products/` của URL do bucket cung cấp — key có prefix → `/products/products/x` 404) → `{url:"/media/products/<uuid>.<ext>"}`. FE: nút upload trong ProductFormPage images tab (task 1).
- Related (runtime endpoint, ADR): `GET /api/catalog/products/{slug}/related?size=8` — public. `EsEngine.related(slug, size)`: fetch doc theo slug → `more_like_this` trên `name.vi, name.en, description.vi, description.en` (**min_term_freq=1** — spec-critic P1: =2 với tên sản phẩm ít trùng từ → rỗng; min_doc_freq=1 — seed chỉ 24 docs; max_query_terms 12) + filter `status:PUBLISHED` + `must_not _id self` → hydrate `ProductCardDto` qua PG (pattern `hydrateCards`). **Fill cùng category tới N** khi MLT < size (seed nhỏ, MLT chỉ gợi ý). Fallback PgFtsEngine/ES-down: cùng category mới nhất. Slug lạ → page rỗng 200 — PDP ẩn section khi empty.
- CSV export catalog: `GET /api/catalog/admin/products/export.csv` (ADMIN, `text/csv`, `Content-Disposition attachment`) — stream `StreamingResponseBody`, batch 500, cột: id,slug,name_vi,brand,price,compare_price,discount_percent,status,rating_avg,created_at. UTF-8 BOM (3 byte EF BB BF — assert byte trong IT, chứng minh Excel VN).
- CSV export ordering: `GET /api/ordering/admin/orders/export.csv` (ADMIN) — cột: order_id,created_at,status,payment_method,subtotal,discount,shipping_fee,total,currency,coupon_code,items_count. Cùng pattern stream + BOM.

### 2.4 cart-service (A4 abandoned)
- `CartDocument` thêm `String email` (nullable, additive — cart cũ không có → null, bỏ qua sweep đến lần mutation sau có email; Jackson deserialize thiếu field → null, không 500). `CartController`: **mọi path authenticated** — mutation items + đặc biệt `POST /cart/merge` (login-merge: giỏ chỉ qua merge nếu không set email sẽ không bao giờ đủ điều kiện "bỏ quên") — set `email` từ JWT claim.
...
- **ordering**: `SagaTest` thêm `codCheckout_confirmedWithoutStripe` — createOrder(cod) không WireMock stripe → status CONFIRMED, clientSecret null, order.confirmed payload validate schema, coupon finalized; `codDeliver_capturesPayment_emitsOrderPaid` — ship → deliver → **payment DB có đúng 1 intent `cod:<orderId>` SUCCEEDED (replay deliver 2 lần vẫn 1)** + ordering outbox có order.paid row (plan-critic P1: ordering→payment là HTTP thật giữa 2 context thật — không phải WireMock); `codCancelByAdmin_skipRefund` — cancel CONFIRMED COD → không gọi refund, order CANCELLED. `csvExportIT` — ADMIN stream CSV: header 11 cột + ≥1 dòng + **3 byte đầu = UTF-8 BOM**, customer → 403.
