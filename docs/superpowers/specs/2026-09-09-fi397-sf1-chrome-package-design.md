...
## 4. CSS chiến lược
- `chrome/styles.css` (export `./styles.css`): `.chrome-header*`, `.chrome-icon-btn`, `.chrome-vh`, `.um-*`, `.chrome-site-footer*` (tên class chủ đích KHÔNG dùng `.site-footer*` — storefront app.css còn sở hữu tới SF-4, tránh đè nhau khi cả 2 stylesheet load), `.chrome-container` (1240px/padding 16 — P1 critic: footer self-contained, không phụ thuộc `.container` của storefront) (port values từ shell `header.css` + account `page.css` + storefront `app.css` tương ứng — CHỈ var(--*), không hex, **KHÔNG `url()`** — gotcha FI-390: css shared package qua MF dev → spa-fallback HTML → resource error; assets nếu cần qua JS FontFace pattern). Shell `main.tsx` import thêm `@ecommerce/chrome/styles.css` (touch map ✓).
- **Ngoại lệ §4 được duyệt (review W4 round 1 — P1 AA):** footer được phép dùng **chrome-local custom properties** `--chrome-footer-{bg,link,link-dark,heading}` pin GIÁ TRỊ LEGACY FI-390 §2.2.5 (bg #212121 · link #bbb, dark #a0a0a0 · heading #ffffff — copy từ storefront app.css, KHÔNG phải hex mới). LÝ DO định lượng: `var(--c-text)` flip trong dark → footer sáng #F5F5F5 lệch FI-390 + contrast link 2.5:1 (dark) / 3.5:1 (light) FAIL AA 4.5:1; pin legacy giữ footer tối cả 2 theme khớp storefront (8.4:1 light / 6.1:1 dark — PASS). Footer là surface duy nhất được exception này; mọi màu khác vẫn var(--*) token thuần. Clarify (T13): `var(--token, legacy-fallback)` kế thừa từ nguồn được phép, không cần per-case exception.
