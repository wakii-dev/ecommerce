...
1. Sync matrix e2e spec MỚI `frontend/e2e/tests/session-sync-matrix.spec.ts` — 2 pages CÙNG Playwright context × 2 app (storefront Next + shell Vite cùng origin): login A→B, logout A→B, 2FA challenge (setup→enable→login→challenge KHÔNG broadcast→hoàn tất→broadcast), OAuth callback (error-path không corrupt state; success-path = seam setToken giống login + unit ref), 20-run rotate-race 0 spurious logout (cross-app). Spec env-driven (chạy được cả dev-rig lẫn isolated :8480).
2. One-origin golden path walkthrough qua entry dev (rig của worktree này): home→PLP→PDP→cart→checkout (guest COD; Stripe nếu 3-gates sống)→confirmation→account + /admin (layout admin RIÊNG, không chrome) — Rule 0, coordinator tự nhìn.
...
5. Docker full regression ISOLATED +400 (compose project `fi397sf5`, override `docker-compose.override-sf2.yml` có sẵn từ SF-2 — tái sử dụng): build + boot toàn stack, entry `:8480` toàn flow OK + /admin; health từng service; chạy sync-matrix spec trên :8480 (prod-mode same-origin proof theo recipe pack SF-2).
...
11. Sweeps: `git diff <fork>..HEAD -- frontend/pnpm-lock.yaml` rỗng (0 dep mới); `git diff <fork>..HEAD -- backend/ contracts/` chỉ được là comment-only (hiện: gateway-routes.yml comment SF-3 — verify lại + ghi báo cáo).
...
| Backend dùng chung :8080 cho rig A có hợp lệ? | Có — diff backend fork→HEAD = comment-only (đã verify `git diff a9a8fad..` ) | sweep item 11 |
