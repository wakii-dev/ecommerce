...
# 2. Mint token (identity 3600s → token 1 giờ)
...
# 3. Ghi override + recreate ordering với token mới
...
YML
docker compose -p fi397sf5 "${OVF[@]}" -f scripts/qa/docker-override-sf5-token.yml --profile full up -d ordering-service 2>&1 | tail -1
