...
const CURATED = new Map([
  ...
  ['ordering-service|HttpCatalogPricingClient|GET /api/catalog/products/by-id/{id}',
    { token: 'none', note: 'SF-4 FI-408 — default by-id-path public (FI-397 compose ORDERING_PRICING_BYIDPATH, giờ default khớp); PUBLISHED-only — draft/deleted 404 → ItemUnavailableException' }],
  ...
  ['partner-api|CatalogClient|GET /api/catalog/products', { token: 'none', note: 'public permitAll — không gắn header (defaultHeader admin-token đã xóa SF-4/FI-408)' }],
  ['partner-api|CatalogClient|GET /api/catalog/search', { token: 'none', note: 'như list' }],
  ['partner-api|CatalogClient|GET /api/catalog/categories', { token: 'none', note: 'như list' }],
  ['partner-api|CatalogClient|GET /api/catalog/products/{slug}', { token: 'none', note: 'như list' }],
  ['partner-api|CatalogClient|GET /api/catalog/products/by-id/{id}',
    { token: 'none', note: 'public by-id PUBLISHED-only — UUID-lookup hồi sinh (GAP-3 interim admin-token kết thúc SF-4/FI-408); draft/deleted 404 → null → partner NOT_FOUND' }],
